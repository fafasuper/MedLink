...
def load_gz_file_into_df(path: Path, dtype: Optional[dict] = None):
    ...
    # Read the gzipped CSV in chunks
    with pd.read_csv(download_dir / f"{stemmed_filename}.csv.gz", compression="gzip", dtype=dtype,
                     chunksize=100) as reader:
        for chunk in reader:
            file = pd.concat([file, chunk], ignore_index=True)
